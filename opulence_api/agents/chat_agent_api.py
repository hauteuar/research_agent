...
API-Based Version - Uses HTTP calls instead of direct model loading
...
# Remove vLLM imports - using API calls instead
# from vllm import AsyncLLMEngine, SamplingParams

from .base_agent import BaseOpulenceAgent
...
class OpulenceChatAgent(BaseOpulenceAgent):
    """Enhanced chat agent for natural language interaction with mainframe analysis - API-based"""

    def __init__(self, coordinator, llm_engine=None,
                 db_path: str = "opulence_data.db", gpu_id: int = 0):

        # Call parent constructor - API coordinator handles engine management
        super().__init__(coordinator, "chat_agent", db_path, gpu_id)

        # Store coordinator reference for API calls
        self.api_coordinator = coordinator
    ...
    async def _generate_with_api(self, prompt: str, params: Dict[str, Any] = None) -> str:
        """Generate text using API coordinator instead of direct LLM"""
        try:
            # Default parameters for text generation
            api_params = {
                "max_tokens": params.get("max_tokens", 512) if params else 512,
                "temperature": params.get("temperature", 0.3) if params else 0.3,
                "top_p": params.get("top_p", 0.9) if params else 0.9,
                "stop": params.get("stop", []) if params else [],
                "stream": False
            }
            ...
            # Call API coordinator
            result = await self.api_coordinator.call_model_api(
                prompt=prompt,
                params=api_params,
            ...
            if not generated_text:
                ...
                self.logger.warning("API returned empty response, using fallback")
                return "I'm processing your request, but encountered an issue generating a detailed response."

            return generated_text

        except Exception as e:
            self.logger.error(f"API generation failed: {str(e)}")
            # Return fallback response instead of empty string
    ...
    async def process_chat_query(self, query: str, conversation_history: List[Dict] = None) -> Dict[str, Any]:
        """Process a chat query using API-based generation"""

        try:
            # Clean conversation history to remove any slice objects
            clean_history = []
            ...
            # Add processing info - updated for API coordinator
            result['api_based'] = True
            result['agent_type'] = self.agent_type
            result['coordinator_type'] = 'api_based'

            # Get server info if available
            if hasattr(self.api_coordinator, 'load_balancer'):
                result['available_servers'] = len(self.api_coordinator.load_balancer.get_available_servers())
                result['total_servers'] = len(self.api_coordinator.load_balancer.servers)
    ...
    async def _handle_general_query(self, context: ChatContext) -> Dict[str, Any]:
        ...
        # Generate contextual response using API
    ...
    async def _generate_analysis_response(self, component: str, analysis: Dict, user_query: str) -> str:
        """Generate intelligent analysis response using API calls"""
...
"""
        ...
        try:
            response = await self._generate_with_api(prompt, params)
            return response.strip()
        except Exception as e:
            self.logger.error(f"API response generation failed: {str(e)}")
    ...
    async def _generate_lineage_response(self, component: str, lineage_result: Dict, user_query: str) -> str:
        """Generate intelligent lineage response using API calls"""
...
"""
    ...
    async def _generate_search_response(self, search_terms: List[str], search_results: List[Dict], user_query: str) -> str:
        """Generate intelligent search response using API calls"""
...
"""
    ...
    async def _generate_comparison_response(self, comp1: str, comp2: str, analysis1: Dict, analysis2: Dict, user_query: str) -> str:
        """Generate intelligent comparison response using API calls"""
...
"""
    ...
    async def _generate_impact_response(self, component: str, analysis: Dict, user_query: str) -> str:
        """Generate intelligent impact analysis response using API calls"""
...
"""
    ...
    async def _generate_contextual_response(self, context: ChatContext) -> str:
        """Generate contextual response for general queries using API calls"""
...
Just describe what you'd like to know about your mainframe systems!
    ...
    # ==================== API-Based Context Manager ====================

    def get_engine_context(self):
        """Return API-based context manager for compatibility"""
        return self._api_engine_context()

    async def _api_engine_context(self):
        """API-based context manager that doesn't manage engines"""
        class APIEngineContext:
            def __init__(self, chat_agent):
                self.chat_agent = chat_agent
                self.logger = chat_agent.logger

            async def __aenter__(self):
                # No engine loading needed - API coordinator handles this
                return self
            ...
            async def generate(self, prompt: str, sampling_params, request_id: str = None):
                """Generate text via API coordinator (compatible with vLLM interface)"""
                ...
                # Call API coordinator
                result = await self.chat_agent.api_coordinator.call_model_api(
                    prompt=prompt,
                    params=params,
                    preferred_gpu_id=self.chat_agent.gpu_id
                )
        ...
        return APIEngineContext(self)

    # ==================== Cleanup and Resource Management ====================

    def cleanup(self):
        """Clean up chat agent resources - API version"""
        # No direct engine management needed with API coordinator
        # Just clear any cached data

        self.logger.info("🧹 Cleaning up API-based ChatAgent resources...")

        # Clear any internal caches
        if hasattr(self, '_cache'):
            self._cache.clear()

        # Reset coordinator reference
        self.api_coordinator = None

        self.logger.info("✅ API-based ChatAgent resources cleaned up")

    def __repr__(self):
        return (f"OpulenceChatAgent(API-based, "
                f"agent_type={self.agent_type}, "
                f"coordinator_type={'api_based' if hasattr(self, 'api_coordinator') else 'unknown'})")

    # ==================== Backwards Compatibility Methods ====================

    async def _generate_with_llm(self, prompt: str, sampling_params) -> str:
        """Backwards compatibility method - redirects to API calls"""
        self.logger.warning("_generate_with_llm called - redirecting to API-based generation")
        ...
        return await self._generate_with_api(prompt, params)
    ...
    def get_agent_status(self) -> Dict[str, Any]:
        ...
        return {
            "agent_type": self.agent_type,
            "coordinator_type": "api_based",
            "api_coordinator_available": self.api_coordinator is not None,
            "database_path": self.db_path,
            "query_patterns_loaded": len(self.query_patterns),
            "knowledge_base_loaded": len(self.knowledge_base),
            "status": "ready" if self.api_coordinator else "no_coordinator"
        }

    async def test_api_connection(self) -> Dict[str, Any]:
        """Test API coordinator connection"""
        try:
            test_prompt = "Hello, this is a test."
            test_params = {"max_tokens": 10, "temperature": 0.1}

            start_time = time.time()
            result = await self._generate_with_api(test_prompt, test_params)
            response_time = time.time() - start_time

            return {
                "status": "success",
                "response_time": response_time,
                "api_response_length": len(result),
    ...
    async def batch_generate_responses(self, prompts: List[str], params: Dict[str, Any] = None) -> List[str]:
        """Generate multiple responses using API calls - useful for batch processing"""
        responses = []
        ...
        for prompt in prompts:
            try:
                ...
                self.logger.error(f"Batch generation failed for prompt: {str(e)}")
                responses.append(f"Error generating response: {str(e)}")

        return responses

    async def get_model_server_info(self) -> Dict[str, Any]:
        """Get information about available model servers"""
        try:
            if hasattr(self.api_coordinator, 'load_balancer'):
                servers_info = []
                for server in self.api_coordinator.load_balancer.servers:
                    servers_info.append({
                ...
                return {
                    ...
                    "load_balancing_strategy": self.api_coordinator.config.load_balancing_strategy.value
                }
            else:
    ...
    async def switch_preferred_server(self, gpu_id: int) -> Dict[str, Any]:
        """Switch preferred server for this agent"""
        try:
            if hasattr(self.api_coordinator, 'load_balancer'):
                server = self.api_coordinator.load_balancer.get_server_by_gpu_id(gpu_id)
                if server:
                    old_gpu_id = self.gpu_id
                    self.gpu_id = gpu_id
                    return {
                        "status": "success",
                        "old_gpu_id": old_gpu_id,
                        "new_gpu_id": gpu_id,
                    ...
                    return {
                        "status": "failed",
                        "error": f"No server found for GPU ID {gpu_id}"
                    }
            else:
                return {
                    "status": "failed",
