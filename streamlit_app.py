# streamlit_app.py
"""
Opulence Deep Research Mainframe Agent - Streamlit UI (Dual GPU Version)
Updated for DualGPUOpulenceCoordinator
"""
...
st.set_page_config(
    page_title="Opulence - Dual GPU Deep Research Agent",
    page_icon="🧠",
    layout="wide",
...
# Try to import dual GPU coordinator with error handling
try:
    from opulence_coordinator import (
        DualGPUOpulenceCoordinator,
        DualGPUOpulenceConfig,
        SingleGPUChatEnhancer,  # Keep same name - works with dual GPU
...
def show_debug_info():
    ...
    if st.session_state.debug_mode:
        ...
        debug_info = {
            ...
            "coordinator_type": "dual_gpu" if st.session_state.coordinator else None,
        ...
        if st.session_state.coordinator:
            try:
                ...
                debug_info.update({
                    "selected_gpus": health.get('selected_gpus'),
                    "gpu_locked": health.get('status') == 'healthy',
...
async def init_dual_gpu_coordinator():
    ...
    if st.session_state.coordinator is None:
        try:
            ...
            try:
                ...
                st.session_state.coordinator = create_dual_gpu_coordinator(force_gpu_ids=[1, 2])
                st.session_state.initialization_status = "completed"
                return True
        except Exception as e:
...
def process_chat_query(query: str) -> Dict[str, Any]:
    ...
    if not COORDINATOR_AVAILABLE:
        return {
            "response": "❌ Dual GPU Coordinator not available. Please check the import error in debug mode.",
            "response_type": "error",
            "suggestions": []
        }
    ...
    try:
        ...
        # Process with the dual GPU coordinator
        ...
        if isinstance(result, dict):
            ...
            return {
                ...
                "suggestions": [],
                "gpus_used": getattr(st.session_state.coordinator, 'selected_gpus', 'unknown')
            }
        ...
        try:
            ...
            if len(gpu_ids) >= 2:
                ...
                with col1:
                    ...
                    try:
                        ...
                        st.metric("Total Tasks", gpu_status_0.get('total_tasks_processed', 0))
                    except:
                        st.metric("Memory Used", "N/A")
                ...
                with col2:
                    ...
                    try:
                        ...
                        st.metric("Total Tasks", gpu_status_1.get('total_tasks_processed', 0))
                    except:
                        st.metric("Memory Used", "N/A")
                        st.metric("Active Tasks", "N/A")
            ...
            with col1:
                try:
                    ...
                    st.metric("Total Tasks Completed", "N/A")
            ...
            with col3:
                st.metric("GPU Count", len(gpu_ids))
...
def show_enhanced_chat_analysis():
    """Enhanced chat analysis interface for dual GPU"""
    st.markdown('<div class="sub-header">💬 Chat with Dual GPU Opulence</div>', unsafe_allow_html=True)
    ...
    if st.session_state.coordinator:
        try:
            health = st.session_state.coordinator.get_health_status()
            gpu_ids = health.get('selected_gpus', [])
            st.success(f"🟢 Chat Agent Ready on GPUs {gpu_ids}")
        except:
            ...
            if message["role"] == "user":
                ...
                with st.chat_message("assistant"):
                    ...
                    if isinstance(message.get("content"), dict):
                        ...
                        gpus_used = response_data.get("gpus_used")
                        if gpus_used:
                            st.caption(f"🎯 Processed on GPUs {gpus_used}")
    ...
    if user_input:
        ...
        with st.spinner("🧠 Dual GPU Opulence is thinking..."):
...
def process_uploaded_files(uploaded_files):
    """Process uploaded files using the dual GPU coordinator"""
    if not st.session_state.coordinator:
    ...
    try:
        ...
        # Process files in batch using dual GPU coordinator
        status_text.text("Processing files with Dual GPU Opulence...")
        ...
        with results_container:
            if isinstance(result, dict) and result.get("status") == "success":
                gpus_used = result.get('gpus_used', 'Unknown')
                st.success(f"✅ Successfully processed {result.get('files_processed', 0)} files in {result.get('processing_time', 0):.2f} seconds on GPUs {gpus_used}")
        ...
        st.session_state.processing_history.append({
            ...
            "gpus_used": result.get("gpus_used", "unknown") if isinstance(result, dict) else "unknown"
        })
...
def analyze_component_dual_gpu(component_name: str, component_type: str, user_question: str = None, chat_enhanced: bool = True):
    """Enhanced component analysis using dual GPU coordinator"""
    if not st.session_state.coordinator:
        st.error("System not initialized")
        return
    
    with st.spinner(f"🧠 Analyzing {component_name} on dual GPU..."):
        try:
            if chat_enhanced and user_question:
                # Use chat-enhanced analysis if available
                try:
                    enhancer = SingleGPUChatEnhancer(st.session_state.coordinator)  # Works with dual GPU too
                    result = safe_run_async(
...
def show_system_health():
    ...
    st.markdown('<div class="sub-header">⚙️ Dual GPU System Health & Statistics</div>', unsafe_allow_html=True)
    
    if not COORDINATOR_AVAILABLE:
        st.error("🔴 System Status: Dual GPU Coordinator Not Available")
        st.markdown("### Import Error")
        st.code(st.session_state.get('import_error', 'Unknown import error'))
        st.info("Please ensure the opulence_coordinator_dual_gpu module is properly installed and configured.")
        return
    
    if not st.session_state.coordinator:
        st.warning("🟡 System Status: Not Initialized")
        if st.button("🔄 Initialize Dual GPU System"):
            with st.spinner("Initializing dual GPU system..."):
                try:
                    success = safe_run_async(init_dual_gpu_coordinator())
                    if success and not isinstance(success, dict):
                        st.success("✅ Dual GPU system initialized successfully")
                        st.rerun()
                    else:
    ...
    try:
        ...
        if st.session_state.processing_history:
            for activity in st.session_state.processing_history[-5:]:
                gpu_info = f" (GPUs {activity.get('gpus_used', 'Unknown')})" if activity.get('gpus_used') else ""
...
def show_file_upload():
    """Show file upload interface for dual GPU system"""
    st.markdown('<div class="sub-header">📂 File Upload & Processing (Dual GPU)</div>', unsafe_allow_html=True)
    
    if not COORDINATOR_AVAILABLE:
        st.warning("⚠️ Dual GPU Coordinator not available. File processing is disabled in demo mode.")
        st.info("To enable file processing, ensure the opulence_coordinator_dual_gpu module is properly installed.")
        return
    ...
    if upload_type == "Single Files":
        ...
        if uploaded_files:
            ...
            if st.button("Process Files on Dual GPU"):
        ...
        if uploaded_zip:
            if st.button("Extract and Process ZIP on Dual GPU"):
...
def process_zip_file(uploaded_zip):
    """Process uploaded ZIP file using dual GPU"""
    if not st.session_state.coordinator:
        st.error("System not initialized")
        return
    ...
    try:
        ...
        st.info(f"Found {len(file_paths)} files to process on dual GPU")
        
        # Process files using dual GPU coordinator
        result = safe_run_async(
            st.session_state.coordinator.process_batch_files(file_paths)
        )
        ...
        if isinstance(result, dict) and result.get("status") == "success":
            gpus_used = result.get('gpus_used', 'Unknown')
            st.success(f"✅ Successfully processed {result.get('files_processed', 0)} files on GPUs {gpus_used}")
        else:
            error_msg = result.get('error', 'Unknown error') if isinstance(result, dict) else str(result)
            st.error(f"❌ Processing failed: {error_msg}")
...
def show_enhanced_component_analysis():
    """Enhanced component analysis with dual GPU chat integration"""
    st.markdown('<div class="sub-header">🔍 Enhanced Component Analysis (Dual GPU)</div>', unsafe_allow_html=True)
    ...
    if st.button("🔍 Analyze Component on Dual GPU") and component_name:
        analyze_component_dual_gpu(component_name, component_type, user_question, chat_enhanced)
...
def show_enhanced_search():
    """Enhanced search interface with dual GPU chat integration"""
    st.markdown('<div class="sub-header">🔍 Enhanced Code Search (Dual GPU)</div>', unsafe_allow_html=True)
    ...
    if st.button("🔍 Search with Dual GPU Enhancement") and search_query:
        perform_enhanced_search_dual_gpu(search_query, search_type, result_count)


def perform_enhanced_search_dual_gpu(search_query: str, search_type: str, result_count: int):
    """Perform enhanced search with dual GPU chat explanations"""
    if not st.session_state.coordinator:
        st.error("System not initialized")
        return
    
    with st.spinner(f"🔍 Searching for '{search_query}' on dual GPU..."):
        try:
            # Use chat-enhanced search if available
            try:
                enhancer = SingleGPUChatEnhancer(st.session_state.coordinator)  # Works with dual GPU
                result = safe_run_async(
            ...
            if result.get("response_type") == "enhanced_search":
                gpus_used = result.get("gpus_used", "Unknown")
                st.success(f"✅ Found {result.get('total_found', 0)} results on GPUs {gpus_used}")
                        ...
                        with st.expander(f"Result {i}: {search_result.get('metadata', {}).get('program_name', 'Unknown')}"):
                            ...
                            if st.button(f"Analyze {metadata.get('program_name', 'Component')}", key=f"analyze_result_{i}"):
                                ...
                                if component_name:
                                    ...
                                    analyze_component_dual_gpu(component_name, "auto-detect", f"Tell me about this component found in search for '{search_query}'", True)
...
def export_chat_history():
    """Export chat history with enhanced formatting including dual GPU info"""
    ...
    export_data = {
        "export_info": {
            ...
            "gpus_used": getattr(st.session_state.coordinator, 'selected_gpus', ['unknown']) if st.session_state.coordinator else ['unknown']
        },
        "conversation": []
    }
    ...
    for message in st.session_state.chat_history:
        if isinstance(message.get("content"), dict):
            ...
            export_data["conversation"].append({
                ...
                "gpus_used": message["content"].get("gpus_used", "unknown"),
    ...
    st.download_button(
        "📥 Download Dual GPU Chat History",
        export_json,
        file_name=f"opulence_dual_gpu_chat_{dt.now().strftime('%Y%m%d_%H%M%S')}.json",
        mime="application/json"
    )


def generate_chat_summary():
    """Generate an intelligent summary of the chat conversation using dual GPU"""
    if not st.session_state.chat_history:
    ...
    with st.spinner("🔄 Generating conversation summary on dual GPU..."):
        try:
            ...
            gpus_used = summary_result.get("gpus_used", "unknown") if isinstance(summary_result, dict) else "unknown"
            
            # Display summary in a nice format
            st.markdown("### 📋 Conversation Summary")
            st.markdown(summary)
            st.caption(f"Generated on GPUs {gpus_used}")
...
def generate_follow_up_suggestions():
    """Generate intelligent follow-up question suggestions using dual GPU"""
    ...
    with st.spinner("🔮 Generating suggestions on dual GPU..."):
        try:
            ...
            if isinstance(suggestion_result, dict):
                ...
                gpus_used = suggestion_result.get("gpus_used", "unknown")
...
def display_enhanced_component_analysis(analysis: dict):
    """Display enhanced component analysis results with dual GPU information"""
    ...
    if analysis.get("response_type") == "enhanced_analysis":
        ...
        gpus_used = analysis.get("gpus_used", "Unknown")
        st.success(f"✅ Enhanced analysis completed for: **{component_name}** on GPUs {gpus_used}")
        ...
        with tab3:
            ...
            if st.button("📄 Export Enhanced Analysis"):
                ...
                st.download_button(
                    ...
                    export_data,
                    file_name=f"enhanced_analysis_{component_name}_gpus{gpus_used}_{dt.now().strftime('%Y%m%d_%H%M%S')}.json",
                    mime="application/json"
                )
...
def display_component_analysis(analysis: dict):
    """Display component analysis results with better error handling and dual GPU info"""
    if isinstance(analysis, dict) and "error" in analysis:
        st.error(f"Analysis error: {analysis['error']}")
    ...
    st.success(f"✅ Analysis completed for {component_type}: **{component_name}** on GPUs {gpus_used}")
...
def show_analysis_overview(analysis: dict):
    """Analysis overview to show actual data with dual GPU info"""
    ...
    with col4:
        gpus_used = analysis.get("gpus_used", "Unknown")
        st.metric("GPUs Used", str(gpus_used))
...
def show_lineage_analysis(analysis: dict):
    """Lineage analysis display with dual GPU info"""
...
def show_impact_analysis(analysis: dict):
    """Impact analysis display with dual GPU info"""
...
def show_analysis_report(analysis: dict):
    """Comprehensive analysis report with dual GPU info"""
    ...
    report_sections.append(f"**GPUs Used**: {gpus_used}")
    ...
    if "processing_time" in analysis:
        ...
        report_sections.append(f"\n**Analysis completed in**: {processing_time:.2f} seconds on GPUs {gpus_used}")
    ...
    if report_content:
        st.download_button(
            "📄 Download Report",
            report_content,
            file_name=f"opulence_analysis_{component_name}_gpus{gpus_used}_{dt.now().strftime('%Y%m%d_%H%M%S')}.md",
            mime="text/markdown"
        )
...
def show_analysis_debug(analysis: dict):
    """Show debug information for analysis with dual GPU info"""
    ...
    gpus_used = analysis.get("gpus_used", "Unknown")
    st.info(f"🎯 Analysis performed on GPUs {gpus_used}")


def show_field_lineage():
    """Show field lineage analysis interface for dual GPU"""
    st.markdown('<div class="sub-header">📊 Field Lineage Analysis (Dual GPU)</div>', unsafe_allow_html=True)
    ...
    if st.button("🔍 Trace Field Lineage on Dual GPU") and field_name:
...
def trace_field_lineage(field_name: str):
    """Trace field lineage using dual GPU"""
    if not st.session_state.coordinator:
        st.error("System not initialized")
        return
    
    with st.spinner(f"Tracing lineage for {field_name} on dual GPU..."):
        try:
...
def display_field_lineage_results(result: dict):
    """Display field lineage results with dual GPU info"""
    ...
    gpus_used = result.get("gpus_used", "Unknown")
    st.success(f"✅ Lineage analysis completed for field: **{field_name}** on GPUs {gpus_used}")
...
def show_db2_comparison():
    """Show DB2 comparison interface for dual GPU"""
    st.markdown('<div class="sub-header">🔄 DB2 Data Comparison (Dual GPU)</div>', unsafe_allow_html=True)
    ...
    if st.button("🔄 Compare Data on Dual GPU") and sqlite_component and db2_component:
        compare_data_sources(sqlite_component, db2_component)


def compare_data_sources(sqlite_comp: str, db2_comp: str):
    """Compare data between SQLite and DB2 using dual GPU"""
    if not st.session_state.coordinator:
        st.error("System not initialized")
        return
    
    with st.spinner("Comparing data sources on dual GPU..."):
        try:
...
def display_comparison_results(result: dict):
    """Display data comparison results with dual GPU info"""
    if isinstance(result, dict) and "error" in result:
    ...
    gpus_used = result.get("gpus_used", "Unknown")
    st.success(f"✅ Data comparison completed on GPUs {gpus_used}")
...
def show_documentation():
    """Show documentation generation interface for dual GPU"""
    st.markdown('<div class="sub-header">📋 Documentation Generation (Dual GPU)</div>', unsafe_allow_html=True)
    ...
    if st.button("📋 Generate Documentation on Dual GPU"):
...
def generate_documentation(doc_type: str, component_name: str, include_diagrams: bool, include_sample_data: bool):
    """Generate documentation using dual GPU"""
    if not st.session_state.coordinator:
        st.error("System not initialized")
        return
    
    with st.spinner("Generating documentation on dual GPU..."):
        try:
...
def display_generated_documentation(result: dict):
    """Display generated documentation with dual GPU info"""
    if isinstance(result, dict) and "error" in result:
    ...
    gpus_used = result.get("gpus_used", "Unknown")
    st.success(f"✅ Documentation generated successfully on GPUs {gpus_used}")
    ...
    if "documentation" in result:
        ...
        st.download_button(
            ...
            file_name=f"opulence_documentation_gpus{gpus_used}_{dt.now().strftime('%Y%m%d_%H%M%S')}.md",
            mime="text/markdown"
        )


def show_example_queries():
    """Show example queries in sidebar with dual GPU info"""
...
def show_footer():
    """Show footer with system information including dual GPU details"""
    st.markdown("---")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        st.markdown("**🧠 Opulence Dual GPU Research Agent**")
        st.markdown("Powered by vLLM, FAISS, and ChromaDB")
        if st.session_state.coordinator:
            gpu_ids = getattr(st.session_state.coordinator, 'selected_gpus', ['Unknown'])
            st.markdown(f"Running on GPUs {gpu_ids}")
...
if __name__ == "__main__":
    try:
        ...
        if health.get("status") == "healthy":
            ...
            st.error("🔴 Dual GPU System Status: Issues Detected")
        ...
        with col4:
            gpu_count = len(health.get("selected_gpus", []))
...
def rebuild_indices():
    """Rebuild vector indices using dual GPU"""
    if not st.session_state.coordinator:
        st.error("System not initialized")
        return
    
    with st.spinner("Rebuilding indices on dual GPU..."):
        try:
...
def export_system_logs():
    ...
    try:
        # Read log file
        log_file = Path("opulence_dual_gpu.log")
        if log_file.exists():
            with open(log_file, 'r') as f:
                log_content = f.read()
            
            st.download_button(
                "📥 Download Dual GPU System Logs",
                log_content,
                file_name=f"opulence_dual_gpu_logs_{dt.now().strftime('%Y%m%d_%H%M%S')}.log",
                mime="text/plain"
            )
        else:
