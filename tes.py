# =================== COMPLETE FILE LIFECYCLE TRACKING ===================
    ...
    async def _store_complete_file_lifecycle(self, file_name: str, lifecycle_data: Dict[str, Any]):
        ...
        try:
            # Create complete file lifecycle table
            cursor.execute("""
    ...
    async def _generate_db2_integration_insights(self, file_name: str, db2_integration: Dict[str, Any]) -> str:
        ...
        try:
            ...
            return f"DB2 integration for {file_name}: {sum([integration_summary['file_to_db2_operations'], integration_summary['db2_to_file_operations']])} integration operations identified."

    # =================== ENHANCED CHAT WITH COMPLETE FILE LIFECYCLE ===================
    
    async def chat_query_complete_file_lifecycle(self, query: str, session_id: str = None) -> Dict[str, Any]:
        """Chat query with complete file lifecycle support"""
        if not session_id:
        ...
        try:
            ...
            # Enhanced query intent analysis for file lifecycle
            query_intent = await self._analyze_file_lifecycle_query_intent(query)
            
            # Get file lifecycle context
            context = await self._get_file_lifecycle_context(query, query_intent)
            
            # Generate response based on file lifecycle intent
            if query_intent == 'complete_file_lifecycle':
                response = await self._handle_complete_file_lifecycle_query(query, context)
            elif query_intent == 'file_creation_lifecycle':
                response = await self._handle_file_creation_query(query, context)
            elif query_intent == 'file_data_journey':
                response = await self._handle_file_data_journey_query(query, context)
            elif query_intent == 'file_db2_integration':
                response = await self._handle_file_db2_integration_query(query, context)
            elif query_intent == 'file_purge_lifecycle':
                response = await self._handle_file_purge_lifecycle_query(query, context)
            else:
                # Fall back to enhanced chat
                return await self.chat_query_with_lifecycle(query, session_id)
            
            # Store file lifecycle chat history
            await self._store_file_lifecycle_chat_history(session_id, query, response, context, query_intent)
            ...
            return {
                ...
                'file_lifecycle_enhanced': True
            }
            
        except Exception as e:
            logger.error(f"❌ File lifecycle chat query failed: {e}")
            return {
                'response': f"I encountered an error analyzing file lifecycle: {str(e)}",
                'intent': 'error',
                'session_id': session_id,
                'file_lifecycle_enhanced': True
            }
    
    async def _analyze_file_lifecycle_query_intent(self, query: str) -> str:
        """Analyze query intent for file lifecycle operations"""
        query_lower = query.lower()
        
        # Complete file lifecycle
        if any(word in query_lower for word in ['complete lifecycle', 'full lifecycle', 'file lifecycle', 'creation to purge']):
            return 'complete_file_lifecycle'
        
        # File creation and initialization
        elif any(word in query_lower for word in ['creation', 'created', 'initialize', 'generate', 'build file']):
            return 'file_creation_lifecycle'
        
        # Data journey and flow
        elif any(word in query_lower for word in ['data journey', 'data flow', 'file journey', 'data movement']):
            return 'file_data_journey'
        
        # DB2 integration
        elif any(word in query_lower for word in ['db2', 'database', 'copy to db2', 'file to database', 'table sync']):
            return 'file_db2_integration'
        
        # File purge and cleanup
        elif any(word in query_lower for word in ['purge', 'delete', 'cleanup', 'archive', 'remove file']):
            return 'file_purge_lifecycle'
        
        else:
            return 'general_file'
    
    async def _get_file_lifecycle_context(self, query: str, intent: str) -> Dict[str, Any]:
        """Get file lifecycle context from stored analysis"""
        context = {}
        
        try:
            # Extract file names from query
            potential_files = self._extract_component_names_from_query(query)
            
            if potential_files:
                file_name = potential_files[0]
                
                # Get complete file lifecycle data
                conn = sqlite3.connect(self.db_path)
                cursor = conn.cursor()
                
                try:
                    # Check if complete file lifecycle table exists
                    cursor.execute("""
                        SELECT name FROM sqlite_master 
                        WHERE type='table' AND name='complete_file_lifecycle'
                    """)
                    
                    if cursor.fetchone():
                        cursor.execute("""
                            SELECT lifecycle_stages, data_journey, db2_integration, 
                                   lifecycle_report, lifecycle_completeness, 
                                   data_flow_complexity, integration_score
                            FROM complete_file_lifecycle 
                        ...
                        if result:
                            context['complete_file_lifecycle'] = {
                                'lifecycle_stages': json.loads(result[0]) if result[0] else {},
                                'data_journey': json.loads(result[1]) if result[1] else {},
                                'db2_integration': json.loads(result[2]) if result[2] else {},
                                'lifecycle_report': result[3],
                                'lifecycle_completeness': result[4],
                                'data_flow_complexity': result[5],
                                'integration_score': result[6]
                            }
            ...
            logger.warning(f"⚠️ File lifecycle context failed: {e}")
        
        return context
    
    async def _handle_complete_file_lifecycle_query(self, query: str, context: Dict[str, Any]) -> str:
        """Handle complete file lifecycle queries"""
        try:
            lifecycle_info = ""
            if 'complete_file_lifecycle' in context:
                lifecycle_data = context['complete_file_lifecycle']
                
                # Summarize lifecycle stages
                stages = lifecycle_data.get('lifecycle_stages', {}).get('stages', {})
                stage_summary = [f"{stage}: {len(ops)} operations" for stage, ops in stages.items() if ops]
                lifecycle_info += f"Lifecycle Stages: {', '.join(stage_summary)}\n"
                
                # Add completeness info
                completeness = lifecycle_data.get('lifecycle_completeness', 0)
                lifecycle_info += f"Lifecycle Completeness: {completeness:.1%}\n"
                
                # Add complexity info
                complexity = lifecycle_data.get('data_flow_complexity', 'Unknown')
                lifecycle_info += f"Data Flow Complexity: {complexity}\n"
                
                # Add integration info
                integration_score = lifecycle_data.get('integration_score', 0)
                lifecycle_info += f"Integration Score: {integration_score:.2f}\n"
            
            prompt = f"""
            Answer this complete file lifecycle question: {query}
            
            File Lifecycle Information:
            {lifecycle_info}
            
            Provide comprehensive information about the complete file lifecycle including:
            - File creation and initialization processes
            - Data population and validation stages
            - Active usage patterns and updates
            - Integration with DB2 and other systems
            - Archival and purge procedures
            - Business impact and dependencies
            - Optimization recommendations
            
            Focus on the complete journey from creation to purge.
            """
            ...
            return f"I found file lifecycle information but couldn't generate a detailed response: {str(e)}"
    
    async def _handle_file_creation_query(self, query: str, context: Dict[str, Any]) -> str:
        """Handle file creation lifecycle queries"""
        try:
            creation_info = ""
            if 'complete_file_lifecycle' in context:
                lifecycle_data = context['complete_file_lifecycle']
                ...
                creation_ops = stages.get('creation', [])
                population_ops = stages.get('population', [])
                
                if creation_ops:
                    creation_info += f"Creation Operations: {len(creation_ops)} found\n"
                    for op in creation_ops[:3]:  # Show first 3
                        creation_info += f"- {op.get('program', 'Unknown')}: {op.get('business_purpose', 'File creation')}\n"
                
                if population_ops:
                    creation_info += f"Population Operations: {len(population_ops)} found\n"
            
            prompt = f"""
            Answer this file creation lifecycle question: {query}
            
            File Creation Information:
            {creation_info}
            
            Provide detailed information about:
            - How the file is initially created
            - Data population and initialization process
            - Validation and quality checks during creation
            - Dependencies and prerequisites
            - Error handling during creation
            - Business rules applied during initialization
            """
            ...
            return f"I found file creation information but couldn't generate a detailed response: {str(e)}"
    
    async def _handle_file_data_journey_query(self, query: str, context: Dict[str, Any]) -> str:
        """Handle file data journey queries"""
        try:
            journey_info = ""
            if 'complete_file_lifecycle' in context:
                lifecycle_data = context['complete_file_lifecycle']
                journey = lifecycle_data.get('data_journey', {}).get('journey_map', {})
                
                sources = journey.get('data_sources', [])
                destinations = journey.get('data_destinations', [])
                transformations = journey.get('transformation_points', [])
                
                journey_info += f"Data Sources: {len(sources)}\n"
                journey_info += f"Data Destinations: {len(destinations)}\n"
                journey_info += f"Transformation Points: {len(transformations)}\n"
                
                if sources:
                    journey_info += f"Key Sources: {', '.join([s.get('source', 'Unknown') for s in sources[:3]])}\n"
                
                if destinations:
                    journey_info += f"Key Destinations: {', '.join([d.get('destination', 'Unknown') for d in destinations[:3]])}\n"
            
            prompt = f"""
            Answer this file data journey question: {query}
            
            Data Journey Information:
            {journey_info}
            
            Provide detailed information about:
            - Complete data flow from sources to destinations
            - Data transformation and processing stages
            - Integration points with external systems
            - Data validation and quality checkpoints
            - Performance characteristics of data movement
            - Business value of data transformations
            """
            ...
            return f"I found data journey information but couldn't generate a detailed response: {str(e)}"
    
    async def _handle_file_db2_integration_query(self, query: str, context: Dict[str, Any]) -> str:
        """Handle file DB2 integration queries"""
        try:
            db2_info = ""
            if 'complete_file_lifecycle' in context:
                lifecycle_data = context['complete_file_lifecycle']
                db2_integration = lifecycle_data.get('db2_integration', {}).get('integration_details', {})
                
                file_to_db2 = db2_integration.get('file_to_db2_operations', [])
                db2_to_file = db2_integration.get('db2_to_file_operations', [])
                
                db2_info += f"File-to-DB2 Operations: {len(file_to_db2)}\n"
                db2_info += f"DB2-to-File Operations: {len(db2_to_file)}\n"
                
                # Add complexity info
                complexity = lifecycle_data.get('db2_integration', {}).get('integration_complexity', 'Unknown')
                db2_info += f"Integration Complexity: {complexity}\n"
                
                # Add synchronization risk
                sync_risk = lifecycle_data.get('db2_integration', {}).get('data_synchronization_risk', 'Unknown')
                db2_info += f"Synchronization Risk: {sync_risk}\n"
            
            prompt = f"""
            Answer this file DB2 integration question: {query}
            
            DB2 Integration Information:
            {db2_info}
            
            Provide detailed information about:
            - File to DB2 data synchronization processes
            - DB2 to file data extraction procedures  
            - Data transformation between file and database formats
            - Synchronization frequency and timing
            ...
            return f"I found DB2 integration information but couldn't generate a detailed response: {str(e)}"
    
    async def _handle_file_purge_lifecycle_query(self, query: str, context: Dict[str, Any]) -> str:
        """Handle file purge and cleanup lifecycle queries"""
        try:
            purge_info = ""
            if 'complete_file_lifecycle' in context:
                lifecycle_data = context['complete_file_lifecycle']
                ...
                archival_ops = stages.get('archival', [])
                purge_ops = stages.get('purge', [])
                backup_ops = stages.get('backup_restore', [])
                
                purge_info += f"Archival Operations: {len(archival_ops)}\n"
                purge_info += f"Purge Operations: {len(purge_ops)}\n"
                purge_info += f"Backup/Restore Operations: {len(backup_ops)}\n"
                
                if archival_ops:
                    purge_info += f"Archival Programs: {', '.join([op.get('program', 'Unknown') for op in archival_ops[:3]])}\n"
            
            prompt = f"""
            Answer this file purge lifecycle question: {query}
            
            Purge/Cleanup Information:
            {purge_info}
            
            Provide detailed information about:
            - File archival procedures and timing
            - Data purge policies and retention periods
            - Backup and recovery mechanisms
            - Compliance requirements for data retention
            - Dependencies that must be considered before purging
            - Recovery procedures if purged data is needed
            - Audit trail and logging of purge activities
            """
            ...
            return f"I found purge lifecycle information but couldn't generate a detailed response: {str(e)}"
    
    async def _store_file_lifecycle_chat_history(self, session_id: str, query: str, response: str, 
                                                context: Dict[str, Any], intent: str):
        """Store file lifecycle chat history"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO file_lifecycle_chat_history 
                (session_id, user_query, system_response, query_intent, context_data)
                VALUES (?, ?, ?, ?, ?)
            """, (session_id, query, response, intent, json.dumps(context)))
            
            conn.commit()
        except Exception as e:
            logger.warning(f"⚠️ File lifecycle chat history storage failed: {e}")
        finally:
            conn.close()    # =================== ENHANCED STREAMLIT UI WITH LIFECYCLE SUPPORT ===================
    
    async def enhanced_lifecycle_analysis_page(self):
        """Enhanced lifecycle analysis page with CICS/DB2 support"""
        st.header("🔄 Complete System Lifecycle Analysis")
        
        program_name = st.text_input("Enter program name for complete lifecycle analysis:")
        
        analysis_type = st.selectbox(
            "Choose analysis type:",
            ["Complete System Lifecycle", "CICS Transaction Lifecycle", "DB2 Data Lifecycle", "All Components"]
        )
        
        if program_name and st.button("Analyze System Lifecycle"):
            with st.spinner(f"Analyzing {program_name} lifecycle..."):
                if analysis_type == "Complete System Lifecycle":
                    analysis = await self.analyzer.analyze_complete_system_lifecycle(program_name)
                elif analysis_type == "CICS Transaction Lifecycle":
                    analysis = await self.analyzer.analyze_cics_transaction_lifecycle(program_name)
                elif analysis_type == "DB2 Data Lifecycle":
                    analysis = await self.analyzer.analyze_db2_data_lifecycle(program_name)
                else:  # All Components
                    cobol_analysis = await self.analyzer.get_component_analysis(program_name)
                    cics_analysis = await self.analyzer.analyze_cics_transaction_lifecycle(program_name)
                    db2_analysis = await self.analyzer.analyze_db2_data_lifecycle(program_name)
            ...
            if analysis.get('status') == 'success':
                st.success(f"✅ Lifecycle analysis complete for {program_name}")
                
                # Display based on analysis type
                if analysis_type == "Complete System Lifecycle":
                    await self._display_complete_lifecycle_analysis(analysis)
                elif analysis_type == "CICS Transaction Lifecycle":
                    await self._display_cics_lifecycle_analysis(analysis)
                elif analysis_type == "DB2 Data Lifecycle":
                    await self._display_db2_lifecycle_analysis(analysis)
                else:
                    await self._display_all_component_analyses(analysis)
    ...
    async def _display_complete_lifecycle_analysis(self, analysis: Dict[str, Any]):
        ...
        if 'system_overview' in lifecycle_data:
            ...
            with col2:
                st.write("**Recovery Mechanisms:**")
                for mechanism in error_handling.get('recovery_mechanisms', []):
                    st.write(f"• {mechanism}")
        
        # Performance Characteristics
        if 'performance_characteristics' in cics_data:
            st.subheader("⚡ Performance Characteristics")
            performance = cics_data['performance_characteristics']
    ...
    async def _display_db2_lifecycle_analysis(self, analysis: Dict[str, Any]):
        ...
        # Table Operations
        if 'table_operations' in db2_data:
            st.subheader("📊 Table Operations")
            table_ops = db2_data['table_operations']
            
            table_data = []
            for table_name, operations in table_ops.items():
                table_data.append({
                    'Table': table_name,
                    'Operations': ', '.join(operations.get('operations', [])),
                    'Access Frequency': operations.get('access_frequency', 'N/A'),
                    'Data Volume': operations.get('data_volume', 'N/A'),
                    'Business Criticality': operations.get('business_criticality', 'N/A')
                })
        ...
        if 'data_transformations' in db2_data:
            st.subheader("🔄 Data Transformations")
            transformations = db2_data['data_transformations']
            
            for i, transform in enumerate(transformations, 1):
                with st.expander(f"Transformation #{i}: {transform.get('source', 'Unknown')} → {transform.get('target', 'Unknown')}"):
                    st.write(f"**Transformation Logic:** {transform.get('transformation_logic', 'N/A')}")
                    st.write(f"**Validation Rules:** {transform.get('validation_rules', 'N/A')}")
        
        # Performance Considerations
        if 'performance_considerations' in db2_data:
            st.subheader("⚡ Performance Considerations")
            performance = db2_data['performance_considerations']
    ...
    async def _display_all_component_analyses(self, analysis: Dict[str, Any]):
        ...
        with tab1:
            ...
            if cobol_analysis.get('status') == 'success':
                # Display COBOL analysis (reuse existing logic)
    ...
    async def enhanced_chat_interface_page(self):
        """Enhanced chat interface with CICS/DB2 support"""
        ...
        # Chat input with enhanced suggestions
        user_query = st.text_input("Ask me about your mainframe system:")
        
        if user_query and st.button("Send"):
            # Add user query to history
            st.session_state.enhanced_chat_history.append({"role": "user", "content": user_query})
            
            # Get enhanced AI response
            with st.spinner("Analyzing with enhanced CICS/DB2 support..."):
                response = await self.analyzer.chat_query_with_lifecycle(user_query)
        ...
        # Clear chat button
        if st.button("Clear Enhanced Chat History"):
            st.session_state.enhanced_chat_history = []
        ...
        with col1:
            st.write("**CICS & Transaction Analysis:**")
            cics_suggestions = [
                "Analyze CICS transaction lifecycle for CUSTOMER-TXN",
                "What CICS resources does ORDER-PROC use?",
                "Show me the complete transaction flow",
                "How does error handling work in CICS programs?"
            ]
            
            for suggestion in cics_suggestions:
                if st.button(suggestion, key=f"cics_{suggestion}"):
                    st.session_state.enhanced_chat_history.append({"role": "user", "content": suggestion})
                    with st.spinner("Processing CICS query..."):
                        response = await self.analyzer.chat_query_with_lifecycle(suggestion)
                    st.session_state.enhanced_chat_history.append({
                        "role": "assistant", 
                        "content": response['response'],
                        "intent": response.get('intent', 'cics_lifecycle'),
                        "enhanced": True
                    })
                    st.experimental_rerun()
        
        with col2:
            st.write("**DB2 & Data Analysis:**")
            db2_suggestions = [
                "Analyze DB2 data lifecycle for PAYMENT-PROC",
                "What tables does ACCOUNT-UPDATE access?",
                "Show me the complete data journey",
                "How is transaction integrity maintained?"
            ]
            
            for suggestion in db2_suggestions:
                if st.button(suggestion, key=f"db2_{suggestion}"):
                    st.session_state.enhanced_chat_history.append({"role": "user", "content": suggestion})
                    with st.spinner("Processing DB2 query..."):
                        response = await self.analyzer.chat_query_with_lifecycle(suggestion)
                    st.session_state.enhanced_chat_history.append({
                        "role": "assistant", 
                        "content": response['response'],
                        "intent": response.get('intent', 'db2_lifecycle'),
                        "enhanced": True
                    })
                    st.experimental_rerun()
        
        # Complete lifecycle suggestions
        st.write("**Complete System Analysis:**")
        complete_suggestions = [
            "Give me the complete system lifecycle for TRADE-SETTLEMENT",
            "How do COBOL, CICS, and DB2 work together in this system?",
            "What's the end-to-end data flow and business impact?"
        ]
        
        for suggestion in complete_suggestions:
            if st.button(suggestion, key=f"complete_{suggestion}"):
                st.session_state.enhanced_chat_history.append({"role": "user", "content": suggestion})
                with st.spinner("Processing complete lifecycle query..."):
                    response = await self.analyzer.chat_query_with_lifecycle(suggestion)
                st.session_state.enhanced_chat_history.append({
                    "role": "assistant", 
                    "content": response['response'],
                    "intent": response.get('intent', 'complete_lifecycle'),
                    "enhanced": True
                })
                st.experimental_rerun()
    ...
    def enhanced_cli_interface():
        """Enhanced CLI with CICS/DB2 lifecycle support"""
        import argparse
        
        parser = argparse.ArgumentParser(description="Enhanced Opulence Mainframe Analyzer CLI")
        ...
        async def process_enhanced_cli():
            results = {}
            ...
            if args.chat:
                ...
                result = await analyzer.chat_query_with_lifecycle(args.chat)
        ...
        asyncio.run(process_enhanced_cli())
    
    def create_enhanced_sample_data():
        """Create enhanced sample files with CICS and DB2 operations"""
        enhanced_cobol = '''
           IDENTIFICATION DIVISION.
           PROGRAM-ID. ENHANCED-SAMPLE.
        ...
        enhanced_jcl = '''
//ENHANCEDJOB JOB (ACCT),'ENHANCED SAMPLE JOB',CLASS=A,MSGCLASS=X
//*
//* Enhanced JCL with DB2 and CICS operations
//*
//STEP1    EXEC PGM=ENHANCED-SAMPLE,REGION=8M
...
//OUTFILE  DD   DSN=PROD.ENHANCED.OUTPUT,
//              DISP=(NEW,CATLG,DELETE),
//              SPACE=(TRK,(100,10))
//*
//STEP2    EXEC PGM=DSNTEP2
//SYSTSIN  DD   *
  RUN PROGRAM(ENHANCED-SAMPLE) PLAN(CUSTPLAN)
/*
//SYSPRINT DD   SYSOUT=*
//SYSTSPDB DD   DSN=DB2.CUSTOMER.DATABASE,DISP=SHR
        ...
        Path("enhanced_samples").mkdir(exist_ok=True)
        
        with open("enhanced_samples/enhanced_sample.cbl", "w") as f:
            f.write(enhanced_cobol)
        
        with open("enhanced_samples/enhanced_sample.jcl", "w") as f:
            f.write(enhanced_jcl)
        
        with open("enhanced_samples/enhanced_customer.cpy", "w") as f:
            f.write(enhanced_copybook)
        
        print("✅ Enhanced sample files created in 'enhanced_samples' directory")
    ...
    # Update the main run method to include enhanced lifecycle analysis
    async def run(self):
        """Run the enhanced Streamlit application"""
        st.title("🏢 Opulence Mainframe Analyzer - Enhanced Edition")
        st.markdown("**Complete COBOL/CICS/DB2 Analysis with AI-Powered Lifecycle Insights**")
        
        # Enhanced sidebar navigation
        page = st.sidebar.selectbox(
            "Choose Analysis Type",
            ["File Upload & Analysis", "Component Analysis", "Enhanced Lifecycle Analysis", 
             "Field Lineage", "Code Search", "Enhanced Chat Interface", "System Overview"]
        )
        
        if page == "File Upload & Analysis":
            await self.file_upload_page()
        elif page == "Component Analysis":
            await self.component_analysis_page()
        elif page == "Enhanced Lifecycle Analysis":
            await self.enhanced_lifecycle_analysis_page()
        elif page == "Field Lineage":
            await self.field_lineage_page()
        elif page == "Code Search":
            await self.code_search_page()
        elif page == "Enhanced Chat Interface":
            await self.enhanced_chat_interface_page()
        elif page == "System Overview":
            await self.system_overview_page()
...
__all__ = [
    ...
    'create_enhanced_sample_data'
        ...
        # Data Journey
        if 'data_journey' in lifecycle_data:
            st.subheader("📈 Data Journey")
            journey = lifecycle_data['data_journey']
            
            # Create data flow diagram
            flow_data = []
            for phase_name, phase_data in journey.items():
                if isinstance(phase_data, dict):
                    for key, value in phase_data.items():
                        if isinstance(value, list):
                            flow_data.extend([(phase_name, key, item) for item in value])
                        else:
                            flow_data.append((phase_name, key, str(value)))
            
            if flow_data:
                flow_df = pd.DataFrame(flow_data, columns=['Phase', 'Aspect', 'Detail'])
                st.dataframe(flow_df, use_container_width=True)
        
        # Performance Profile
        if 'performance_profile' in lifecycle_data:
            st.subheader("⚡ Performance Profile")
            performance = lifecycle_data['performance_profile']
            
            perf_metrics = []
            for key, value in performance.items():
                perf_metrics.append({'Metric': key.replace('_', ' ').title(), 'Value': str(value)})
            
            perf_df = pd.DataFrame(perf_metrics)
            st.dataframe(perf_df, use_container_width=True)
        
        # Business Impact
        if 'business_impact_analysis' in lifecycle_data:
            st.subheader("💼 Business Impact Analysis")
            business_impact = lifecycle_data['business_impact_analysis']
        ...
        if 'optimization_opportunities' in lifecycle_data:
            st.subheader("🚀 Optimization Opportunities")
            opportunities = lifecycle_data['optimization_opportunities']
            
            for i, opp in enumerate(opportunities, 1):
                with st.expander(f"Opportunity #{i}: {opp.get('area', 'Unknown')}"):
                    st.write(f"**Recommendation:** {opp.get('recommendation', 'N/A')}")
                    st.write(f"**Estimated Impact:** {opp.get('estimated_impact', 'N/A')}")
                    st.write(f"**Implementation Effort:** {opp.get('implementation_effort', 'N/A')}")
    ...
    async def _display_cics_lifecycle_analysis(self, analysis: Dict[str, Any]):
        ...
        # CICS Resources
        if 'cics_resources_used' in cics_data:
            st.subheader("🔧 CICS Resources Used")
            resources = cics_data['cics_resources_used']
            
            for resource_type, resource_list in resources.items():
                if resource_list and resource_list != False:
                    if isinstance(resource_list, list):
                        st.write(f"**{resource_type.title()}:** {', '.join(resource_list)}")
                    else:
                        st.write(f"**{resource_type.title()}:** {resource_list}")
        
        # Error Handling
        if 'error_handling' in cics_data:
            st.subheader("⚠️ Error Handling")
            error_handling = cics_data['error_handling']
            
            col1, col2 = st.columns(2)
            with col1:
                st.write("**Error Conditions:**")
                for condition in error_handling.get('error_conditions', []):
    ...
    async def _store_cics_analysis(self, program_name: str, analysis: Dict[str, Any]):
        ...
        try:
            # Create CICS analysis table if not exists
            cursor.execute("""
    ...
    async def _store_db2_analysis(self, program_name: str, analysis: Dict[str, Any]):
        ...
        try:
            # Create DB2 analysis table if not exists
            cursor.execute("""
    ...
    async def _store_complete_lifecycle_analysis(self, program_name: str, analysis: Dict[str, Any]):
        ...
        try:
            # Create complete analysis table if not exists
            cursor.execute("""
    ...
    # =================== ENHANCED CHAT WITH LIFECYCLE SUPPORT ===================
    
    async def chat_query_with_lifecycle(self, query: str, session_id: str = None) -> Dict[str, Any]:
        """Enhanced chat query with CICS/DB2 lifecycle support"""
        if not session_id:
            session_id = str(uuid.uuid4())
        ...
        try:
            ...
            query_intent = await self._analyze_enhanced_query_intent(query)
            
            # Get enhanced context including CICS/DB2
            context = await self._get_enhanced_query_context(query, query_intent)
            
            # Generate response based on enhanced intent
            if query_intent == 'cics_lifecycle':
            ...
            elif query_intent == 'component_analysis':
                response = await self._handle_enhanced_component_analysis_query(query, context)
            elif query_intent == 'field_lineage':
                response = await self._handle_enhanced_field_lineage_query(query, context)
            elif query_intent == 'code_search':
                response = await self._handle_enhanced_code_search_query(query, context)
            else:
                response = await self._handle_enhanced_general_query(query, context)
            
            # Store enhanced chat history
            await self._store_enhanced_chat_history(session_id, query, response, context, query_intent)
            ...
            logger.error(f"❌ Enhanced chat query failed: {e}")
            return {
    ...
    async def _analyze_enhanced_query_intent(self, query: str) -> str:
        """Analyze query intent with CICS/DB2 support"""
        query_lower = query.lower()
    ...
    async def _get_enhanced_query_context(self, query: str, intent: str) -> Dict[str, Any]:
        """Get enhanced context including CICS/DB2 data"""
        context = {}
        ...
        try:
            ...
            if potential_components:
                ...
                if intent == 'cics_lifecycle':
                    ...
                    # Get basic context plus CICS/DB2 if available
                    context = await self._get_component_analysis_context(component_name)
                    context.update(await self._get_cics_context(component_name))
                    context.update(await self._get_db2_context(component_name))
            
        except Exception as e:
            logger.warning(f"⚠️ Enhanced context retrieval failed: {e}")
    ...
    async def _get_cics_context(self, component_name: str) -> Dict[str, Any]:
        ...
        try:
            # Check if CICS analysis table exists
            cursor.execute("""
                SELECT name FROM sqlite_master 
                WHERE type='table' AND name='cics_analysis'
            """)
    ...
    async def _get_db2_context(self, component_name: str) -> Dict[str, Any]:
        ...
        try:
            # Check if DB2 analysis table exists
            cursor.execute("""
                SELECT name FROM sqlite_master 
                WHERE type='table' AND name='db2_analysis'
            """)
    ...
    async def _get_complete_lifecycle_context(self, component_name: str) -> Dict[str, Any]:
        ...
        try:
            # Check if complete analysis table exists
            cursor.execute("""
                SELECT name FROM sqlite_master 
                WHERE type='table' AND name='complete_lifecycle_analysis'
            """)
    ...
    async def _handle_enhanced_component_analysis_query(self, query: str, context: Dict[str, Any]) -> str:
        """Handle enhanced component analysis queries"""
        try:
            component_info = ""
            ...
            # Add CICS info if available
            if 'cics_operations' in context:
                component_info += f"CICS Operations: {', '.join(context['cics_operations'])}\n"
            
            # Add DB2 info if available
            if 'db2_operations' in context:
                component_info += f"DB2 Operations: {', '.join(context['db2_operations'])}\n"
            
            if 'database_tables' in context:
                component_info += f"Database Tables: {', '.join(context['database_tables'])}\n"
            
            prompt = f"""
            Answer this enhanced component analysis question: {query}
            
            Component Information:
            {component_info}
            
            Provide comprehensive analysis including COBOL structure, CICS transaction processing,
            DB2 data operations, and overall system integration.
            """
            ...
            return f"I found component information but couldn't generate a detailed response: {str(e)}"
    
    async def _handle_enhanced_field_lineage_query(self, query: str, context: Dict[str, Any]) -> str:
        """Handle enhanced field lineage queries with CICS/DB2 context"""
        return await self._handle_field_lineage_query(query, context)
    
    async def _handle_enhanced_code_search_query(self, query: str, context: Dict[str, Any]) -> str:
        """Handle enhanced code search queries with CICS/DB2 context"""
        return await self._handle_code_search_query(query, context)
    
    async def _handle_enhanced_general_query(self, query: str, context: Dict[str, Any]) -> str:
        """Handle enhanced general queries with CICS/DB2 knowledge"""
        try:
            prompt = f"""
    ...
    async def _store_enhanced_chat_history(self, session_id: str, query: str, response: str, 
                                         context: Dict[str, Any], intent: str):
        """Store enhanced chat history with intent and context"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        try:
            # Update chat history table to include intent
            cursor.execute("""
            ...
            logger.warning(f"⚠️ Enhanced chat history storage failed: {e}")
        finally:
            conn.close()# opulence_complete_analyzer.py
"""
...
class OpulenceMainframeAnalyzer:
    ...
    def _analyze_query_intent(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ['analyze', 'analysis', 'what does', 'explain']):
            return 'component_analysis'
            ...
            context['chunks'] = [
                {
                    ...
                    'metadata': json.loads(chunk[2]) if chunk[2] else {}
                }
                for chunk in chunks
            ]
    ...
    async def _handle_general_query(self, query: str, context: Dict[str, Any]) -> str:
        ...
        try:
            prompt = f"""
            You are an expert mainframe analysis assistant. Answer this question: {query}
            
            Provide helpful information about mainframe development, COBOL, JCL, or data analysis.
            If you need specific component information, ask the user to specify the component name.
            """
            
            sampling_params = SamplingParams(temperature=0.4, max_tokens=250)
            request_id = str(uuid.uuid4())
            ...
            return f"I can help with mainframe analysis questions. Could you please rephrase your question? Error: {str(e)}"
    ...
    async def get_cached_file_analysis(self, file_name: str) -> Optional[Dict[str, Any]]:
        """Get cached file analysis"""
        return await self._get_cached_file_analysis(file_name)
...
class OpulenceStreamlitUI:
    """Streamlit UI for Opulence Mainframe Analyzer"""
    
    def __init__(self, analyzer: OpulenceMainframeAnalyzer):
    ...
    def setup_page_config(self):
        ...
        st.set_page_config(
            page_title="Opulence Mainframe Analyzer",
            page_icon="🏢",
            layout="wide",
    ...
    async def run(self):
        ...
        st.title("🏢 Opulence Mainframe Analyzer")
        st.markdown("**Complete COBOL/JCL/Copybook Analysis with AI-Powered Insights**")
        
        # Sidebar navigation
        page = st.sidebar.selectbox(
            "Choose Analysis Type",
            ["File Upload & Analysis", "Component Analysis", "Field Lineage", 
             "Code Search", "Chat Interface", "System Overview"]
        )
        ...
        if page == "File Upload & Analysis":
            ...
            await self.chat_interface_page()
        elif page == "System Overview":
            await self.system_overview_page()
    
    async def file_upload_page(self):
        """File upload and analysis page"""
        st.header("📁 File Upload & Analysis")
        
        # File uploader
        uploaded_files = st.file_uploader(
            "Upload COBOL, JCL, or Copybook files",
            type=['cbl', 'cob', 'jcl', 'cpy', 'copy'],
            ...
            for uploaded_file in uploaded_files:
                with st.expander(f"📄 {uploaded_file.name}"):
                    ...
                    with st.spinner(f"Analyzing {uploaded_file.name}..."):
                        result = await self.analyzer.load_and_parse_file(temp_path)
                    ...
                    if result['status'] == 'success':
                        ...
                        if result.get('chunks'):
                            ...
                            st.bar_chart(chunk_types)
                        
                        # Show lineage analysis
                        if result.get('lineage_analysis'):
                            st.subheader("📊 Field Lineage Analysis")
                            lineage = result['lineage_analysis']
                            if 'field_lineage' in lineage:
                                st.write(f"**Total fields analyzed:** {lineage.get('total_fields_analyzed', 0)}")
                                st.write(f"**Fields with multiple references:** {lineage.get('fields_with_multiple_refs', 0)}")
                        
                        # Show lifecycle analysis
                        if result.get('lifecycle_analysis'):
                            st.subheader("🔄 Lifecycle Analysis")
                            lifecycle = result['lifecycle_analysis']
                            if lifecycle.get('business_purpose'):
                                st.write(f"**Business Purpose:** {lifecycle['business_purpose']}")
    ...
    async def component_analysis_page(self):
        """Component analysis page"""
        st.header("🔍 Component Analysis")
        
        component_name = st.text_input("Enter component name (program, file, or field):")
        
        if component_name and st.button("Analyze Component"):
            with st.spinner(f"Analyzing {component_name}..."):
                analysis = await self.analyzer.get_component_analysis(component_name)
            
            if analysis['status'] == 'success':
                st.success(f"✅ Analysis complete for {component_name}")
                ...
                with col2:
                    ...
                    st.metric("Complexity Rating", complexity.get('complexity_rating', 'N/A'))
                with col3:
                    field_summary = analysis.get('field_usage_summary', {})
                    st.metric("Unique Fields", field_summary.get('total_unique_fields', 0))
                with col4:
                    dead_fields = analysis.get('dead_field_analysis', {})
                    st.metric("Dead Fields", dead_fields.get('dead_field_count', 0))
                
                # Chunk breakdown chart
                if analysis.get('chunk_breakdown'):
                    st.subheader("📊 Chunk Type Breakdown")
                    chart_data = pd.DataFrame(
                        list(analysis['chunk_breakdown'].items()),
                        columns=['Chunk Type', 'Count']
                    )
                    fig = px.pie(chart_data, values='Count', names='Chunk Type', 
                               title="Distribution of Chunk Types")
                    st.plotly_chart(fig, use_container_width=True)
                
                # Field usage analysis
                if analysis.get('dead_field_analysis'):
                    st.subheader("🔍 Field Usage Analysis")
                    dead_analysis = analysis['dead_field_analysis']
                    
                    field_data = pd.DataFrame({
                        'Field Type': ['Active Fields', 'Read-Only Fields', 'Dead Fields'],
                        'Count': [
                            dead_analysis.get('active_field_count', 0),
                            dead_analysis.get('read_only_count', 0),
                            dead_analysis.get('dead_field_count', 0)
                        ]
                    })
                    
                    fig = px.bar(field_data, x='Field Type', y='Count', 
                               title="Field Usage Distribution",
                               color='Field Type')
                    st.plotly_chart(fig, use_container_width=True)
                    
                    # Show dead fields if any
                    if dead_analysis.get('dead_fields'):
                        st.warning("⚠️ **Potentially Dead Fields Found:**")
                        st.write(", ".join(dead_analysis['dead_fields'][:10]))
            ...
            elif analysis['status'] == 'not_found':
                st.warning(f"⚠️ Component '{component_name}' not found. Please check the name or upload the file first.")
        ...
        if field_name and st.button("Trace Field Lineage"):
            ...
            if 'error' not in lineage:
                ...
                if lineage.get('usage_by_operation'):
                    ...
                    usage_details = []
                    for operation, usages in lineage['usage_by_operation'].items():
                        for usage in usages:
                            usage_details.append({
                                'Operation': operation,
                                'Program': usage['program'],
                                'Context': usage['context'][:100] + "..." if len(usage['context']) > 100 else usage['context'],
                                'Chunk ID': usage['chunk_id']
                            })
                    
                    usage_df = pd.DataFrame(usage_details)
    ...
    async def chat_interface_page(self):
        ...
        # Chat input
        user_query = st.text_input("Ask me anything about your mainframe code:")
        ...
        for message in st.session_state.chat_history:
            if message["role"] == "user":
                ...
                st.write(f"**Assistant {intent_emoji}:** {message['content']}")
        ...
        suggestions = [
            "Analyze the CUSTOMER-PROC program",
            "Trace the lineage of ACCOUNT-ID field",
            "Find all programs that calculate interest",
            "What's the lifecycle of TRADE-FILE?",
            "Show me validation logic patterns"
        ]
        ...
        for suggestion in suggestions:
            if st.button(suggestion, key=f"suggestion_{suggestion}"):
                ...
                with st.spinner("Processing suggestion..."):
                    response = await self.analyzer.chat_query(suggestion)
                st.session_state.chat_history.append({
                    "role": "assistant", 
                    "content": response['response'],
                    "intent": response.get('intent', 'general')
    ...
    async def system_overview_page(self):
        ...
        if 'error' not in overview:
            ...
            if overview.get('chunk_statistics'):
                chunk_df = pd.DataFrame(overview['chunk_statistics'])
                fig3 = px.bar(chunk_df, x='chunk_type', y='count', 
                            title="Code Chunks by Type")
                st.plotly_chart(fig3, use_container_width=True)
            
            # Vector statistics
            st.subheader("🧠 AI Vector Index")
            vector_stats = overview.get('vector_statistics', {})
            col1, col2 = st.columns(2)
            with col1:
                st.metric("Embeddings Created", vector_stats.get('embeddings_created', 0))
            with col2:
                st.metric("Search Index Size", vector_stats.get('search_index_size', 0))
...
def create_sample_data():
    ...
    sample_cobol = '''
       ...
       PROCESS-CUSTOMERS.
           ...
           COMPUTE WS-ACCOUNT-BALANCE = 1000.50
           DISPLAY "Customer processed: " WS-CUSTOMER-NAME.
    '''
    
    sample_jcl = '''
//SAMPLEJOB JOB (ACCT),'SAMPLE JOB',CLASS=A,MSGCLASS=X
//STEP1    EXEC PGM=SAMPLE-PROG
//SYSOUT   DD   SYSOUT=*
//CUSTFILE DD   DSN=PROD.CUSTOMER.DATA,DISP=SHR
//OUTFILE  DD   DSN=PROD.CUSTOMER.OUTPUT,
//              DISP=(NEW,CATLG,DELETE),
//              SPACE=(TRK,(100,10))
    '''
    ...
    with open("samples/sample.cbl", "w") as f:
        f.write(sample_cobol)
    
    with open("samples/sample.jcl", "w") as f:
        f.write(sample_jcl)
    
    with open("samples/customer.cpy", "w") as f:
...
def cli_interface():
    ...
    async def process_cli():
        ...
        if args.analyze:
            print(f"Analyzing component: {args.analyze}")
            result = await analyzer.get_component_analysis(args.analyze)
            results[f"analysis_{args.analyze}"] = result
        
        if args.field:
            print(f"Tracing field lineage: {args.field}")
            result = await analyzer.find_field_usage(args.field)
            results[f"lineage_{args.field}"] = result
        
        if args.search:
            print(f"Searching code: {args.search}")
            result = await analyzer.semantic_search(args.search)
            results[f"search_{args.search}"] = result
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # Streamlit UI mode
        print("🚀 Starting Opulence Mainframe Analyzer...")
        print("📋 Navigate to the Streamlit URL to access the web interface")
